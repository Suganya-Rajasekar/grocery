<?php
namespace App\Http\Controllers\Api\V1\Emperica;
...
class CartController extends Controller
{
	function addCart( Request $request)
	{
		...
		if ($this->method == 'POST') {
			...
			if($foodData->food_type != 'ticket') {
				...
				if($foodData->food_type != 'home_event_menu') {
				...
				if($foodData->food_type == "home_event_menu") {
					if (isset($request->themes) && request('themes') != '')
						$rules['themes']	= ['required','exist_check:homeevent_themes,where:id:=:'.request('themes').'-where:status:=:active'];
					if (isset($request->preferences) && request('preferences') != '') {
						$pref	= explode(',', request('preferences'));
						foreach ($pref as $key => $value) {
							$rules['preferences']	= ['required','exist_check:homeevent_preferences,where:id:=:'.trim($value).'-where:status:=:active'];
						}
					}
					$rules['meals']	= 'required';
					$meals	= $request->meals;
					if($guard == 'api') {
						$meals = json_encode($meals,true);
					}
					$request->merge(['meals' =>json_decode($meals,true)]);
				}
			}
		...
		if ($this->method == 'POST') {
			...
			$foodtype	= clone ($fooditem);
			$foodtype	= $foodtype->food_type;
			if (!empty($fooditem)) {
				if($foodtype == "home_event_menu") {
					$meal 			 = $request->meals;
					$tot_meal_count  = (isset($meal['Vegetarian']) ? $meal['Vegetarian'] : 0) + (isset($meal['Non-Vegetarian']) ? $meal['Non-Vegetarian'] : 0);
					if($tot_meal_count > $request->quantity) {
						$response['message'] = 'Please enter the people count within menu quantity or increase menu quantity.';
						$response['status']  = 422;
 						return \Response::json($response,$response['status']);
					} 
					if($request->quantity < $fooditem->minimum_order_quantity) {
						$response['message'] = 'You can\'t decrease quantity below Minimum Order quantity of home event.';
						$response['status']  = 422;
 						return \Response::json($response,$response['status']);
					}
				} else {			
					if ($userId == 0 && $fooditem->discount > 0 && $fooditem->purchase_quantity > 0 && $fooditem->purchase_quantity_count < request('quantity')) {
						$response['message']	= "You can add only ".$fooditem->purchase_quantity." into your cart";
						return \Response::json($response,422);
					} elseif ($userId > 0 && ($fooditem->discount > 0 && $fooditem->purchase_quantity > 0) && $fooditem->purchase_quantity_count < request('quantity')) {
						$response['message']	= "You have ordered max quantity for this order ! Pl order this dish from our regular Chefs menu";
						$response['status'] = 422;
						return \Response::json($response,422);
					}
				}
			...
			$data['time_slot']	= ($foodtype != "ticket" && $foodtype != "home_event_menu") ? request('time_slot') : '';
			...
			$data['food_type']  = ($foodtype == "ticket") ? $foodtype : (($foodtype == 'home_event_menu') ? 'home_event_menu' : 'menuitem');
			...
			if (isset($request->unit) && null !== $request->unit && $request->unit > 0) {
				...
				$unit_base_price = $foodData->unit_detail[$key]['price'];
				$unit_discount_price = 0;
				if ($foodData->unit_detail[$key]['discount_price'] != 0) {
					$unit	= $foodData->unit_detail[$key]['discount_price'];
					$unit_discount_price = $foodData->unit_detail[$key]['discount_price'];
				}
			}
			...
			if(!empty($chCart->first())){
				...
				if($foodtype == "ticket") {
					...
					} elseif ($cart_foodtype == "home_event_menu") {
						$response['message']  = "Remove the home event menu from the cart to add ticket";
						$response['foodtype'] = "ticket";					
					}
				} elseif($foodtype == "home_event_menu") {
					if($cart_foodtype == "menuitem") {
						$response['message']  = "Remove the fooditem from the cart to add home event menu";
						$response['foodtype'] = "home_event_menu";
					} elseif($cart_foodtype == "ticket") {
						$response['message']  = "Remove the ticket from the cart to add home event menu";
						$response['foodtype'] = "home_event_menu";
					}
				} else {
					if($cart_foodtype == "ticket") {
						...
						$response['message']  = "Remove the home event menu from the cart to add fooditem";
						$response['foodtype'] = "menuitem";
					}
				}
			...
			$price	= ($unit > 0) ? $unit : (($foodData->discount_price != 0) ? $foodData->discount_price : $foodData->price);
			
			$data['price']	    = (request('quantity') * $price);
			$data['discount']   = !empty($foodData->discount) ? $foodData->discount : 0;
			$data['base_price'] = ($unit > 0) ? $unit_base_price : $foodData->price;
			$data['discount_price'] = ($unit > 0) ? $unit_discount_price : (($foodData->discount != 0) ? $foodData->discount_price : 0); 
			$data['theme']	  		= ($foodtype == "home_event_menu") ? (!empty($request->themes) ? $request->themes : '') : '';
			$data['preferences']	= ($foodtype == "home_event_menu") ? (!empty($request->preferences) ? $request->preferences : '') : '';
			$data['meals_count']	= ($foodtype == "home_event_menu") ? (!empty($request->meals) ? json_encode($request->meals) : '') : '';
			if($foodtype == "ticket") {
				$fCart	= $pCart->where('food_id', request('food_id'))->first();
			} elseif($foodtype == "home_event_menu"){
				$fCart	= $pCart->where('food_id', request('food_id'))->where('date',request('date'))->first();
			} else {	
			...
			if ($foodtype == "home_event_menu" && ((request('themes') != 0) && request('themes') > 0)) {
				$cartDat	= Cart::find($cartDetail->id);
				$cartDat->increment('price',(float) $cartDat->theme_data[0]['amount']);
			}
			if (!empty($cartDetail))
			...
			if (request('ucart_id') > 0) {
				...
				if($uCart->food_type == "home_event_menu") {
					if($request->quantity < $uCart->menu_item->minimum_order_quantity) {
						$response['message'] = 'You can\'t decrease quantity below Minimum Order quantity of home event.';
						return \Response::json($response,422);
					}	
				}
				if (!empty($uCart)) {
					if (request('quantity') > 0) {
						...
						if($ucartFood->discount > 0 && $ucartFood->purchase_quantity > 0 && $ucartFood->purchase_quantity <= request('quantity')) {
							$response['message']  = "You have ordered max quantity for this order ! Pl order this dish from our regular Chefs menu";
							return \Response::json($response,422);
						} 
						$fprice		= $addonCart = $atotal = $base_price = $discount_price = $theme_price = 0;		
						if ($uCart->unit != 0) {
							...
							foreach ($units as $key => $value) {
								...
								if($ucartFood->discount != 0) {
									$key = array_search($uCart->unit,array_column($ucartFood->unit_detail,'id'));
									$discount_price = $ucartFood->unit_detail[$key]['discount_price'];
									$fprice = $discount_price;
								}
							}
						} else {
							$fprice = $ucartFood->price;
							$base_price 	= $ucartFood->price;
							if($ucartFood->discount != 0) {
								$discount_price = $ucartFood->discount_price;
								$fprice 		= $ucartFood->discount_price; 	
							}
						}
						...
						if($uCart->food_type == "home_event_menu" && !empty($uCart->theme)) {
							$theme_price = Themes::find($uCart->theme,['id','amount'])->amount;
						}
						$atotal	= $addonCart + $theme_price + (request('quantity') * $fprice);
						$bprice = /*request('quantity') * */$base_price;
						$dprice = /*request('quantity') * */$discount_price;
						$arr	= ['quantity' => request('quantity'), 'price' => $atotal,'base_price' => $bprice,'discount_price' => $dprice];
				...
				if($request->function == "datetime_edit") {
					...
					foreach($cartData as $key=>$value) {
						...
						$value->is_preorder =	(date('Y-m-d') == $request->new_date) ? 'no' : 'yes';
						$value->save();
					}
					$status  = 200;
					$message = "Delivery date and timeslot changed successfully."; 
				}
			}  elseif (isset($request->request_for)) {
				$message	= 'View Cart';
			}
		} elseif ($this->method == 'PUT') {
		...
		if (in_array($this->method, $methods)) {
			...
			$response['DelCharge']			= ($userId == 235 || $userId == 274 || (!empty($response['cart_detail']) && ($response['cart_detail'][0]['vendor_info']->type == "event" || $response['cart_detail'][0]['vendor_info']->type == "home_event"))) ? 0 : $response['subOrder_Count'] * getDelCharge();
			$response['package_charge']		= $userData['packagecharge'];
			$response['DelChargePerOrder']	= ($userId == 235 || $userId == 274 || (!empty($response['cart_detail']) && ($response['cart_detail'][0]['vendor_info']->type == "event" || $response['cart_detail'][0]['vendor_info']->type == "home_event"))) ? 0 : (double) getDelCharge();
		}
		$uCart	= uCartQuery($userId, $cookieID);
		$ucart	= $cucart = clone ($uCart);
		$total_coupon_value = 0;
		$response['used_wallet_amount'] = "0.00";
		...
		if(!empty($ucart->first())) {
			...
			$response['used_wallet_amount']  = 	$ucart->first()->used_wallet_amount;
		}
		$response['userdetails']= ($userId > 0) ? User::find($userId,['id','name','email','mobile']) : (object) [];
		$response['price']		= ($priceTotal - $total_coupon_value - $response['used_wallet_amount']) + $response['DelCharge'] + $resTax + $response['package_charge'];
	...
	public function cartData($userCart,$from='addCart',$restaurant_id=0)
	{
		...
		$Checkdiscount	= clone ($userCart);
		$checkCart	= $checkCart->get();
		$taxTotal	= 0;
		$total_coupon_value = 0;
		$couponData	= $couponData->whereNotIn('coupon_id',[0])->with('getcoupon')->first();
		if (!$checkCart->isEmpty()) {
			foreach($Checkdiscount->get() as $key=> $value){
				if($value->discount != 0) {
					if($value->menu_item) {
						if($value->menu_item->discount == 0) {
							$value->price = $value->base_price * $value->quantity;
							$value->discount_price = 0;
							$value->discount       = 0;
							$value->save();
						} /*elseif ($value->menu_item->discount != $value->discount) {
							$value->discount 		= $value->menu_item->discount;
							$value->price    		= $value->quantity * ($value->base_price - ($value->base_price * ($value->menu_item->discount / 100)));
							$value->discount_price	= $value->base_price * ($value->menu_item->discount / 100);
							$value->save();
						}	*/
					}
				}
			...
			if (!empty($uCart)) {
				...
				foreach ($uCart as $rkey => $rvalue) {
					...
					->get()->append(['food_name','food_price','time_s','eventavailable','theme_data','preferences_data','meals_data']);
	...
	public function cartCount( Request $request)
	{
		...
		if($response['count'] != 0) {		
			...
			foreach($unavailablecart->get() as $key => $value) {
				if($value->ordertimeslotavailable['status'] == false){
