...
<div class="page-header page-header-default">
	<div class="page-header-content">
		<div class="page-title">
			<h5><span class="text-semibold">Master</span> - Time slot categories</h5>
		</div>
	</div>
...
<div class="panel panel-flat">
	...
	<div class="panel-heading">
		<button type="button" class="btn bg-teal-400 btn-labeled btn-rounded" data-toggle="modal" data-target="#modal_cuisine"><b><i class="icon-database-time2"></i></b> {{ ('Add New') }}</button>
	</div>
	@endif
		</div>
		   ...
		   <form class="form-inline" method="GET">
				<div class="form-group mb-2">
					...
					<div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
						...
						<a class="dropdown-item" href="{!! url(getRoleName().'/timecateexport/'.$dn.'?search='.\Request::query('search').'&status='.\Request::query('status')) !!}">{!! $dv !!}</a>
						@endforeach
						</div>
					</div>
				...
				<a href="{!! url('admin/common/timeslotcategory') !!}" class="btn btn-danger font-monserret ml-2 mb-2"><i class="icon-cancel-circle2"></i> Clear</a>
				@endif
			</form>
		</div>
		<br><br>
	...
	<div class="table-responsive-xl">
		<table class="table datatable-responsive table-bordered">
			...
			<tbody>
				...
				@if(count($timeslotcategory)>0)
				@foreach($timeslotcategory as $key=>$value)
				<tr>
					<td>{!! ($key+1)+$page !!}</td>
					<!-- <td>{!!$value->id!!}</td> -->
					<td>{!!$value->name!!}</td>
					...
					<td>
						<div class="text-center d-flex">
							...
							@endif
							@if($access->remove)
								<form action="{!!url(getRoleName().'/timeslotcategory/'.$value->id.'/delete/')!!}" method="post" enctype="Multipart/form-data">
...
<div class="mt-10">
	<div class="pull-left font-montserrat">
		Displaying {{$timeslotcategory->count()+$page}} of {{ $timeslotcategory->total() }} data(s).
	</div>
	<div class="pull-right">
		{{$timeslotcategory->appends(Request::except('page'))->render()}}
	</div>
</div>
<!-- /basic responsive configuration -->
<!-- Cuisine modal -->
	...
	<div class="modal-dialog modal-sm">
		<div class="modal-content">
			<div class="modal-header">
				<h5 class="modal-title">Add/Edit Time slot Category</h5>
				<button type="button" class="close" data-dismiss="modal">&times;</button>
			</div>
			<form action="{!!url(getRoleName().'/timeslotcategory/store')!!}" method="POST" enctype="Multipart/form-data">
