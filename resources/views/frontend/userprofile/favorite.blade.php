<?php // echo "<pre>";print_r($favourites->get_favourites); exit(); ?>
<div class="setting-main-area fav-asw tab-pane fade  verification_area @if(last(request()->segments()) == 'favorite') active show @endif  " id="favorite" style="display: block;opacity: 1;">

    @if(!(\Request::has('fPage')))
    <div class="py-3 d-flex justify-content-between align-items-center">
        <h4 class="d-inline font-weight-bold">Favourites</h4>
        <div class="d-lg-none float-right profile-asw-menu d-inline"><i class="fa fa-bars"></i></div>
    </div>
    @endif
    @if( isset($favourites) && count($favourites->get_favourites) > 0)
    @foreach($favourites->get_favourites as $k => $val)
        @php
        $get_chef=$val->get_vendor_details;
        @endphp 
            ...
            <div class="chef-lists">
                <div class="chefdetails my-3">
                    <div class="d-flex">
                        <div class=" ">
                            <div class="chefimg">
                                <a href="{!!url('/chef/'.$val->vendor_id)!!}" > <img src="{{ $get_chef->avatar }}" alt="">
                        ...
                        <div class="w-100 ml-1 ml-md-3">
                            <div class="o-hid d-flex justify-content-between justify-content-md-start">
                                <div class="w-asw-85">
                                    <div class="chefname">
                                        <a href="{!!url('/chef/'.$val->vendor_id)!!}" ><h2 class="text-black font-opensans mb-0 mb-md-3 elipsis-text font-weight-bold">{{ strip_tags($get_chef->name) }}{{-- <span class="offer-chef">20% OFF</span> --}}</h2></a>
                                        <h4 class="text-muted elipsis-text mb-0 font-montserrat mb-md-3">
                                            <span class="home-span font-montserrat">
                                                @foreach( $get_chef->cuisines as $c1 => $c2)
                                    ...
                                    <div class="sqr-star">
                                        <div class=" star-rating ">
                                            <div class="overflow-hidden profile-asw-rate">
                                                ...
                                                <div>
                                                    <span class="star-points font-montserrat text-black">{{$get_chef->ratings}}</span>
                ...
                @php
                $fooditems=$val->get_all_favorite_alter;
                @endphp
                ...
                <div class="chefsfood">
                    ...
                    <div class="">
                        <div class="owl-carousel owl-theme owl-fav">
                            @foreach($fooditems as $ke => $vals)
                            <div class="item">
                                <div class="chefsfoodlists">
                                    <div class="foodimg">
                                        <a href="{!!url('/menuaddon/'.$vals->id)!!}"><img src="{{ $vals->image }}" alt=""></a>
                                    </div>
                                    <div class="fooddesc">
                                        <h3 class="food-name font-montserrat text-black">
                                            {{ strip_tags($vals->name) }}<i class="fa fa-heart pink-heart"></i>
                                        </h3>
                                        <p class="elipsis-text text-muted font-montserrat">{{ strip_tags($vals->description) }}</p>
                                    </div>
                                    <div class="foodprice">
                                        <h3 class="text-black font-montserrat">&#8377;{{ $vals->price }}</h3>
                                    </div>
                                </div>
                            </div>
                            @endforeach
    ...
    @if(!(\Request::has('fPage')) && $favourites->get_favourites != '' )
    <button class="btn btn-default col-md-12 profileModule"  type="button" name="favourites">Load More</button>
    @endif
    ...
    @else 
        ...
        <div class="text-center">
            ...
            <p class="font-montserrat">No favourites marked yet.</p>
        </div>
        @endif
    @endif
</div>
